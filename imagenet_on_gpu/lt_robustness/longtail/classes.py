...
class IMAGENET_TEST(DatasetFolder):
    ...
    def __init__(self, test_directory, class_2_ix, apply_transform=True):
        augmentation = (
            transforms.Compose(
                [
                    transforms.Resize(256),
        ...
        self.class_2_ix = class_2_ix
        classes = sorted([self.class_2_ix.keys()])
        #
        # classes, self.class_2_ix = self.find_classes(test_directory)

        self.ix_2_class = {v: k for k, v in self.class_2_ix.items()}

        self.dataset = self.make_dataset(test_directory, self.class_2_ix)
        self.dataset_name = "ImageNet"
    ...
    def make_dataset(
        ...
        if not class_2_ix:
            raise ValueError("'class_2_ix' must be passed! ")
