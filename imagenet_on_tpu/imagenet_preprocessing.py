...
def parse_example_proto(example_serialized):
  ...
  ix = tf.cast(features['image/ix'], dtype=tf.int32)
  ...
  return ix, features['image/encoded'], label, bbox
...
def parse_record(raw_record, is_training, dtype):
  ...
  """
  ix, image_buffer, label, bbox = parse_example_proto(raw_record)
  ...
  ix = tf.cast(tf.cast(tf.reshape(ix, shape=[1]), dtype=tf.int32), dtype=tf.float32)

  return ix, image, label
...
def get_parse_record_fn(use_keras_image_data_format=False):
  ...
  def parse_record_fn(raw_record, is_training, dtype):
    ix, image, label = parse_record(raw_record, is_training, dtype)
